...
class TTSDataset(Dataset):
    def __init__(
        ...
        max_audio_length: float = MAX_AUDIO_LENGTH, # Vẫn giữ để lọc file quá lỗi
    ...
    def __getitem__(self, idx):
        ...
        # Load audio (giữ nguyên SR gốc để resample sau cho chính xác)
        audio, sr = librosa.load(sample.audio_path, sr=self.s3gen_sr)
        audio = librosa.util.normalize(audio)
        
        # Resample for S3 tokenizer (16kHz)
        ...
        # Trả về dạng Tensor nhưng KHÔNG pad cứng ở đây, để compute_loss tự xử
        return {
            'audio': torch.FloatTensor(audio),      # Raw audio
            'audio_16k': torch.FloatTensor(audio_16k), # 16k for tokenizer
            'text': text,
...
def load_audio_samples(audio_dir: str, whisper_model) -> List[AudioSample]:
    ...
    for ext in audio_extensions:
        # SỬA LỖI 1: Dùng rglob thay vì glob để tìm file trong thư mục con
    ...
    for audio_path in tqdm(audio_files, desc="Processing audio"):
        try:
            ...
            # SỬA LỖI 2: Chuyển path sang dạng posix (dùng dấu /) để khớp với key trong JSON
            try:
                ...
                # Fallback nếu file không nằm trong audio_dir (hiếm gặp)
            ...
            if audio_path_str in transcript_cache:
                ...
                # print(f"Using cached transcript for {audio_path.name}") # Comment bớt cho đỡ spam log
            else:
